"""
In this module, we define fuctions for a node
"""
...
def hash256(password):
    """
    Compute hash for a string using SHA256
    """
    sha = hashlib.sha256()
...
def user_receive(ip,port,block_path,neighbours):
    """
    Creation of socket for receiving data
    """
    ...
    while True:
        ...
        while True:
            ...
            if flag_broad:
                ...
                if not data:
                    ...
                    if op.checkBlockchain(new_blockchain) and len(new_blockchain)>len_block:
                        ...
                        net.copyFile('../tempchain1.dat',block_path)
                        os.remove('../tempchain1.dat')
            ...
            if data.decode('utf-8').split(',')[0] == 'broad':
                ...
                net.copyFile(block_path,'../tempchain1.dat')
                flag_broad = True
                f = open(block_path, 'w')
                f.close()
            ...
            """
            Receiving a mining request and transmit the block data to the mining function
            """
                global MINING
            ...
            """
            Update blockchain information from its neighbourhood
            """
                try:
    ...
    while True:
        ...
        if option == '1':
            ...
            if payee not in idList:
                ...
                continue
            """
            Request others to mine the block
            """
            else:
            ...
            """
            Update blockchain from neighboorhood
            """
            print('updating...')
            for addr in neighbours:
                try:
...
def user(path_ini,path_blockchain):
    """
    Here we use mutiple thread to do operation( operation() ) and listening( user_receive() )
    """
...
def main():
    user('config/host1.ini','data/blockchain1.dat')
